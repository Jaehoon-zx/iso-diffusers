...
#SBATCH --job-name=diff_74 # Submit a job named "example"
#SBATCH --output=logs/log_74.txt  # 스크립트 실행 결과 std output을 저장할 파일 이름

#SBATCH --partition=a3000
#SBATCH --gres=gpu:3000:1          # Use 1 GPU
...
accelerate launch --mixed_precision="fp16" train_text_to_image.py \
  ...
  --lr_warmup_steps=0 \
  --output_dir="output/sd2-ffhq512-74" \
  --validation_epochs=1 \
