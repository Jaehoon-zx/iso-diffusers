...
from utils import stereographic_proj, inv_stereographic_proj, riemmanian_metric, isometry_loss
...
def main(args):
    ...
    #######################################################################
    def visual_inspection(pipeline, args, accelerator, weight_dtype, epoch):
        logger.info("Visual Inspection & Model saving... ")
        pipeline = pipeline.to(accelerator.device)
        pipeline.set_progress_bar_config(disable=True)
        unet = pipeline.unet
    ...
    for epoch in range(first_epoch, args.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            r = random.random()
            if r < p:
                timesteps = torch.randint(1, int(noise_scheduler.config.num_train_timesteps * p), (bsz,), device=pipeline.device)
                timesteps = timesteps.long()
                iso_cond = False
            else:
                timesteps = torch.randint(int(noise_scheduler.config.num_train_timesteps * p), noise_scheduler.config.num_train_timesteps, (bsz,), device=pipeline.device)
                timesteps = timesteps.long()
                iso_cond = True
